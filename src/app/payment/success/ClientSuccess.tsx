...
export default function ClientSuccess() {
  ...
  const { provider, txRef, refParam } = useMemo(() => {
    const provider = searchParams.get('provider') ?? undefined
    const tx_ref = searchParams.get('tx_ref')
    const session_id = searchParams.get('session_id')
    const txRef = tx_ref ?? session_id ?? undefined
    const refParam = provider === 'stripe' ? 'session_id' : 'tx_ref'
    return { provider, txRef, refParam }
  }, [searchParams])
  ...
  useEffect(() => {
    ...
    const url = `/api/payment/verify?provider=${encodeURIComponent(
      provider
    )}&${refParam}=${encodedRef}`
    ...
    fetch(url, { signal: controller.signal })
      ...
      .catch((err) => { if (err.name !== 'AbortError') setVerified(false) })
      .finally(() => setLoading(false))

    return () => controller.abort()
  }, [provider, txRef, refParam])
        ...
        {loading ? (
          ...
          <>
            ...
            <p className="mt-2 text-xs text-ivory/40">
              Ref: <span className="font-mono">{txRef}</span> · via {provider}
            </p>
          ...
          <>
            ...
            <p className="mt-3 text-ivory/65">
              We couldn&apos;t confirm your payment. If you were charged, please contact support with ref:{' '}
              <strong className="text-gold-light">{txRef ?? '—'}</strong>
            </p>
